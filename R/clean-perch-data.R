...
clean_perch_data <- function(clean_options){
  ...
  datacase <- extract_data_raw(meas_dir,X_strat_nm_tmp_case,X_strat_val_tmp_case,
                                c(BrS_objects,SS_objects),X_extra)
  datactrl <- extract_data_raw(meas_dir,X_strat_nm_tmp_ctrl,X_strat_val_tmp_ctrl,
                                c(BrS_objects,SS_objects),X_extra)
  ...
  if ("ENRLDATE" %in% X_extra){
    ...
    Rdate.case <- lubridate::parse_date_time(datacase$X$ENRLDATE, date_formats)
    Rdate.ctrl <- lubridate::parse_date_time(datactrl$X$ENRLDATE, date_formats)
  ...
  if (length(all_MSS)==0){all_MSS <- NULL}
...
make_meas_object <- function(patho,specimen,test,quality,cause_list){
...
extract_data_raw <- function(meas_dir,strat_nm,strat_val,
  ...
  dat_prepared <- read.csv(meas_dir,header=TRUE,stringsAsFactors=FALSE)
  cleanName    <- colnames(dat_prepared)
  ind_this_strat  <- 1:nrow(dat_prepared)
  for (j in 1:length(strat_nm)){
    ind_this_strat = ind_this_strat[which(dat_prepared[ind_this_strat,strat_nm[j]]==strat_val[[j]] & 
  ...
  lookup_quality <- function(quality_nm){
    if (quality_nm=="BrS"){return(1)}
    if (quality_nm=="SS"){return(2)}
    if (quality_nm=="GS"){return(3)}
  ...
  for (m in seq_along(meas_object)){
    ...
    names(Mobs[[meas_tmp$position]])[curr_pos] <-object_tmp$nm_spec_test
  }
...
show_individual <- function(data_nplcm,ID){
  ...
  for (i in seq_along(data_nplcm$Mobs)){
    Mobs[[i]] <- lapply(data_nplcm$Mobs[[i]],function(df) df[index,])
  }
  list(Mobs = Mobs,
       X    = data$X[index,])
}
