if(getRversion() >= "2.15.1") utils::globalVariables(c("equals","Icat","Z","ThetaBS",
                                                       "Icat.new","Z.new","PsiBS",
                                                       "thetaSS","inprod2",
													   "set_prior_tpr","set_prior_eti"))
...
#     }
    ...
    model_NoReg_BrSandSS_nplcm_ppd <- function(){
      #BEGIN MODEL
    ...
    # the one without ppd:
    model_NoReg_BrSandSS_nplcm <- function(){#BEGIN MODEL
        ## 0) conditional dependence;
        ## 1) accommodates singletons, combos, and NoA;
        ## 2) check-bit to prevent case data informing FPR;
    ...
    if (mcmc_options$ppd==TRUE){
      model_func         <- model_NoReg_BrSandSS_nplcm_ppd
      model_bugfile_name <- "model_NoReg_BrSandSS_nplcm_ppd.bug"
      #file.show(filename)
      # gs <- mybugs("model_NoReg_BrS_plcm_ppd.bug")
    } else {
      model_func         <- model_NoReg_BrSandSS_nplcm
      model_bugfile_name <- "model_NoReg_BrSandSS_nplcm.bug"
