...
must set `individual.pred = TRUE` in MCMC options (see the example of
this function)
...
- x:
  ...
  an `nplcm` object; it contains the file path `DIR_NPLCM` to where the
  model results and specifications are stored. The function first reads
  a list from this folder by
...
specified in `model_options$likelihood$cause_list`; See
