...
Called by [`nplcm()`](https://zhenkewu.com/baker/reference/nplcm.md)
...
positive rates and etiology fractions), initializes the posterior
sampling chain, writes the model file (for `JAGS`), and fits the model.
Features:

- regression (not all discrete covariates);

- nested subclasses, i.e. conditional dependence of multivariate
...
for constructing `.bug` model file; This function then put it in the
